...
class HeroRepostitoryImplementation : HeroRepository {
    ...
    override suspend fun getAlHerroes(page: Int): ApiResponse {
        return ApiResponse(
            success = true,
            message = "ok",
    ...
    override suspend fun searchHeros(page: String?): ApiResponse {
        return ApiResponse(
            success = true,
            message = "ok",
            heroes = findHeroes(query = page)
        )
    }

    override suspend fun searchHerosbyId(id: Int?): ApiResponse {
        return ApiResponse(
            success = true,
            message = "ok",
            heroes = findHeroesbyIds(id = id)
        )
    }
    ...
    private fun findHeroesbyIds(id: Int?): List<Hero> {
        val founded = mutableListOf<Hero>()
        return if (id != null) {
            heroes.forEach { (_, heroes) ->
                heroes.forEach { hero ->
